<!-- Language tabs -->
> [!NOTE]
> <!-- Default to English, tab to switch -->

<details>
<summary>🌐 English</summary>

<p align="center">
  <img src="icon.svg" alt="Backrooms by Andinest Games" width="140">
</p>

<h1 align="center">🟡 BACKROOMS BY ANDINEST GAMES 🟡</h1>

<p align="center">
  <i>Unreality made tangible — a Backrooms survival experience.</i>
</p>

<p align="center">
  <img alt="Godot" src="https://img.shields.io/badge/Engine-Godot%204.7-blue">
  <img alt="Status" src="https://img.shields.io/badge/Status-In%20Development-orange">
  <img alt="License" src="https://img.shields.io/badge/License-MIT-green">
  <img alt="Platform" src="https://img.shields.io/badge/Platform-Windows-lightgrey">
</p>

---

## 🌌 Overview

**Backrooms by Andinest Games** is a first-person Backrooms experience built
with the **Godot 4.7** engine. The world is assembled procedurally around you,
room by room, hallway by hallway — an endless maze of yellow wallpapered limbo
and abandoned office dread.

> [!IMPORTANT]
> The game is still in active development. Expect the occasional bug, flicker,
> and unexplained draft.

---

## 🎮 What's in the Game

### 🗺️ Levels
- **Level 0** — The iconic yellow expanse. Endless wallpapered corridors with a
  low, constant hum and a damp carpet beneath your feet.
- **Level 4** — The abandoned office complex: cubicles, pillars, fluorescent
  lighting, and the feeling that the desks were only just vacated.
- **The Hub** — The central nexus between realities, a momentary refuge before
  you descend deeper.

### 🏚️ Procedural World Generation
- The levels build themselves **chunk by chunk** around the player as you walk,
  giving a truly infinite, seamless maze with no loading breaks.
- A noise-driven layout keeps the experience unpredictable every run.

### 🚶 First-Person Movement
- Full **footstep audio** with a **dynamic head-bob** and **landing pitch**.
- **Sprint**, **crouch**, and **jump** with tight coyote-time and
  jump-buffering.
- **Whistling** — call out into the dark, if you dare.

### 🔦 Lighting & Atmosphere
- **Volumetric fog**, god rays, and **soft dynamic shadows** that carve the
  corridors with believable depth.
- Reflections, screen-space ambient occlusion, and indirect lighting add weight
  to every fluorescent fixture.
- Film-grain and analog glow give the whole game a "found footage" finish.

### 🎚️ Interface
- An animated **video-background main menu** wrapped in a film-grain aesthetic.
- A loading screen with progress bar and cinematic scene-switching.
- A pause menu for the moments you need to breathe.

---

## 🛠️ Engine & Requirements

- **Engine:** Godot 4.7 (Forward+)
- **Platform:** Windows (D3D12)
- **Physics:** Jolt Physics
- **Rendering:** Forward+ with real-time reflections, GI, SSAO/SSIL and
  volumetric fog

---

## 📦 Installation

1. Install [Godot 4.7](https://godotengine.org/download).
2. Open the project by selecting `project.godot` in the Godot Project Manager.
3. Press the **Play** button — and step through the door.

---

## 🙏 Credits

- **Developed by** [Andinest Games](https://github.com/khalilkc)
- **Original prototype by** [TAO71-Games](https://github.com/TAO71-Games/BackroomsWithI4.0-Godot)

---

## 📄 License

Distributed under the **MIT License**. See [`LICENSE.md`](LICENSE.md) for details.

<p align="center">
  <i>Don't look behind you. The walls are listening.</i>
</p>

</details>

---

<details>
<summary>🌍 العربية</summary>

<div dir="rtl" align="center">
  <img src="icon.svg" alt="Backrooms by Andinest Games" width="140">
</div>

<h1 dir="rtl" align="center">🟡 باك رومز بواسطة أندينيست جيمز 🟡</h1>

<p dir="rtl" align="center">
  <i>انعدام الواقع يتحقق — تجربة رعب في عالم الباك رومز.</i>
</p>

<p align="center">
  <img alt="Godot" src="https://img.shields.io/badge/Engine-Godot%204.7-blue">
  <img alt="Status" src="https://img.shields.io/badge/Status-In%20Development-orange">
  <img alt="License" src="https://img.shields.io/badge/License-MIT-green">
  <img alt="Platform" src="https://img.shields.io/badge/Platform-Windows-lightgrey">
</p>

---

## 🌌 نظرة عامة

<div dir="rtl">
**باك رومز بواسطة أندينيست جيمز** هي تجربة رعب من منظور الشخص الأول
مبنية على محرك **Godot 4.7**. يتجمّع العالم إجرائياً من حولك،
غرفةً غرفة وممراً ممراً — متاهة لا نهائية من الفراغ الأصفر ذي الجدران المغطاة
بورق الحائط والرعب المكاتب المهجورة.
</div>

> [!IMPORTANT]
> اللعبة لا تزال قيد التطوير الفعّال. توقّع وجود خطأ عرضي أو وميض
> أو تيّار هواء بلا تفسير.

---

## 🎮 ما الموجود في اللعبة

### 🗺️ المستويات
- **المستوى 0** — الامتداد الأصفر الشهير. ممرات لا نهائية بورق حائط
  مع أزيز منخفض مستمر وسجّادة رطبة تحت قدميك.
- **المستوى 4** — مجمّع المكاتب المهجورة: مقصورات وأعمدة وإضاءة
  فلورية، والشعور بأن المكاتب خُليت للتوّ.
- **ذا هَب** — العقدة المركزية بين الحقائق، ملاذٌ لحظي قبل أن
  تنحدر أعمق.

### 🏚️ توليد العالم الإجرائي
- تبنى المستويات نفسها **قطعة بقطعة** حول اللاعب وأنت تمشي،
  لتعطيك متاهة لا نهائية ومستمرة بلا فواصل تحميل.
- تخطيط مدفوع بالضوضاء يجعل التجربة غير متوقعة في كل مرة.

### 🚶 حركة الشخص
- **صوت خطى** كامل مع **اهتزاز رأس ديناميكي** وحركة هبوط.
- **الركض** و**الانبطاح** و**القفز** مع توقيت مرن وسلس.
- **الصفير** — نادِ في الظلام إن كنت تجرؤ.

### 🔦 الإضاءة والأجواء
- **ضباب حجْمي** وأشعة إلهية وظلال ناعمة ديناميكية تشكّل الممرات
  بعمق مقنع.
- انعكاسات وإضاءة محيطية تمنح كل مصباح فلوري وزناً.
- حُبيبات الفيلم وتوهّج تناظري يعطيان اللعبة لمسة "لقطات قديمة".

### 🎚️ الواجهة
- قائمة رئيسية **بخلفية فيديو** متحركة بأسلوب حُبيبات الفيلم.
- شاشة تحميل بشريط تقدم وانتقالات مشاهد سينمائية.
- قائمة إيقاف مؤقت لتلك اللحظات التي تحتاج فيها إلى التقاط أنفاسك.

---

## 🛠️ المحرك والمتطلبات

- **المحرك:** Godot 4.7 (Forward+)
- **المنصة:** Windows (D3D12)
- **الفيزياء:** Jolt Physics
- **الرسم:** Forward+ مع انعكاسات فورية وإضاءة عامة وSSAO/SSIL
  وضباب حجمي

---

## 📦 التثبيت

1. ثبّت [Godot 4.7](https://godotengine.org/download).
2. افتح المشروع باختيار `project.godot` من مدير المشاريع.
3. اضغط زر **تشغيل** — واعبر الباب.

---

## 🙏 الاعتمادات

- **طوّره** [أندينيست جيمز](https://github.com/khalilkc)
- **النموذج الأصلي** [TAO71-Games](https://github.com/TAO71-Games/BackroomsWithI4.0-Godot)

---

## 📄 الترخيص

موزّع بموجب **رخصة MIT**. راجع [`LICENSE.md`](LICENSE.md) للتفاصيل.

<p dir="rtl" align="center">
  <i>لا تنظر خلفك. الجدران تسمع.</i>
</p>

</details>
